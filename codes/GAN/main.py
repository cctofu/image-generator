...
if __name__ == "__main__":
    ...
    parser.add_argument("--do_interpolated", action="store_true")
    args = parser.parse_args()

    # Global Random Seed
    args.seed = random.randint(2022, 2025)
    ...
    config = f"{args.latent_dim}_{args.generator_hidden_dim}"
    ...
    if args.do_interpolated:
        for j in range(5):  # At least 5 pairs
            z1 = torch.randn(1, args.latent_dim, 1, 1, device=device)
            z2 = torch.randn(1, args.latent_dim, 1, 1, device=device)
            with torch.no_grad():
                interpolated_images = []
                for i in range(11):  # 10 interpolations + 1 endpoint
                    alpha = i / 10
                    z = (1 - alpha) * z1 + alpha * z2
                    interpolated_image = netG(z)
                    interpolated_images.append(interpolated_image)

                interpolated_images = torch.cat(interpolated_images, 0)
                img_grid = make_grid(interpolated_images, nrow=11, normalize=True, value_range=(0, 1))
                save_image(img_grid, f"interpolated_{j}.png")